...
//       try {
//         const response = await fetch('http://localhost:3001/api/data5');
//         if (!response.ok) {
...
//                Sorting Exam Score {sortOrder === 'asc' ? '🔽' : '🔼'}
//               </th>
...
const Fetchj = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cleanedData = slicedData.map(row => row.slice(0, 7)); // Keep only the first 7 columns
        setData(cleanedData);
  ...
  const sortData = (key) => {
    if (key !== 4) {
      // Only sort the Exam Score column (index 4)
      return;
    }
  ...
  const downloadExcel = () => {
    const worksheet = XLSX.utils.aoa_to_sheet([['First Name', 'Last Name', 'Roll No', 'Reg. ID', 'Exam Score', 'Exam Name', 'Action'], ...sortedData]);
        ...
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
            <tr>
              ...
              >
                Exam Score {sortConfig.key === 4 ? (sortConfig.direction === 'asc' ? '🔽' : '🔼') : ''}
              </th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Exam Name</th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((rowData, rowIndex) => (
              <tr key={rowIndex} style={{ border: '1px solid black' }}>
                ...
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[4]}</td> {/* Exam Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[5]}</td> {/* Exam Name */}
                <td style={{ border: '1px solid black', padding: '8px' }}>
                  <button onClick={() => handleViewButtonClick(rowData[6])} className='btn'>
                    View
                  </button>
                </td>
              </tr>
