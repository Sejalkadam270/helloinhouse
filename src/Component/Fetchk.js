...
//       try {
//         const response = await fetch('http://localhost:3001/api/data6');
//         if (!response.ok) {
...
const Fetchk = () => {
  ...
  const sortData = (key) => {
    if (key !== 4) {
      // Only sort the Club Score column (index 4)
      return;
    }
  ...
  const displayData = () => {
    return (
      <div>
        <button onClick={downloadPDF} style={{ marginTop: '80px' }} className='btn'>
          Download as PDF
        </button>
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
