...
//       try {
//         const response = await fetch('http://localhost:3001/api/data8');
//         if (!response.ok) {
...
//                Sorting Sport Score {sortOrder === 'asc' ? '🔽' : '🔼'}
//               </th>
...
const Fetchm = () => {
  ...
  const sortData = (key) => {
    if (key !== 4) {
      // Only sort the Sport Score column (index 4)
      return;
    }
  ...
  const downloadExcel = () => {
    const worksheet = XLSX.utils.aoa_to_sheet([
      ['First Name', 'Last Name', 'Roll No', 'Reg. ID', 'Sport Score', 'Action'],
      ...sortedData
    ]);
        ...
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
            <tr>
              ...
              >
                Sport Score {sortConfig.key === 4 ? (sortConfig.direction === 'asc' ? '🔽' : '🔼') : ''}
              </th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((rowData, rowIndex) => (
              <tr key={rowIndex} style={{ border: '1px solid black' }}>
                ...
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[4]}</td> {/* Sport Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>
