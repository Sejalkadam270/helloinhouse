...
//       try {
//         const response = await fetch('http://localhost:3001/api/data7');
//         if (!response.ok) {
...
//                Sorting Placement Score {sortOrder === 'asc' ? '🔽' : '🔼'}
//               </th>
...
const Fetchl = () => {
  ...
  const sortData = (key) => {
    if (key !== 4) {
      // Only sort the Placement Score column (index 4)
      return;
    }
  ...
  const downloadExcel = () => {
    const worksheet = XLSX.utils.aoa_to_sheet([
      ['First Name', 'Last Name', 'Roll No', 'Reg. ID', 'Placement Score', 'Action'],
      ...sortedData
    ]);
        ...
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
            <tr>
              ...
              >
                Placement Score {sortConfig.key === 4 ? (sortConfig.direction === 'asc' ? '🔽' : '🔼') : ''}
              </th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((rowData, rowIndex) => (
              <tr key={rowIndex} style={{ border: '1px solid black' }}>
                ...
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[4]}</td> {/* Placement Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>
