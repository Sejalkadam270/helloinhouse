...
//                Sorting Patent and Publication Score {sortOrder === 'asc' ? '🔽' : '🔼'}
//               </th>
...
const Fetchg = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cleanedData = rawData.map(row => row.slice(0, 6)); // Keep only first six columns
  ...
  const downloadExcel = () => {
    //const headers = ["First Name", "Last Name", "Roll No", "Reg. ID", "Publication Score", "Patent Score"];
        ...
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
            <tr>
              ...
              >
                Publication Score {sortConfig.key === 4 ? (sortConfig.direction === 'asc' ? '🔽' : '🔼') : ''}
              </th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Patent Score</th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((rowData, rowIndex) => (
              <tr key={rowIndex} style={{ border: '1px solid black' }}>
                ...
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[4]}</td> {/* Publication Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[5]}</td> {/* Patent Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>
