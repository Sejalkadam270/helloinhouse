...
//       try {
//         const response = await fetch('http://localhost:3001/api/data10');
//         if (!response.ok) {
...
//                Sorting Overall Rating Score {sortOrder === 'asc' ? '🔽' : '🔼'}
//               </th>
...
//                   <button onClick={() => handleButtonClick(rowIndex)} className='btn'>view</button>
//                 </td>
//               </tr>
//             ))}
//           </tbody>
...
const Fetchr = () => {
  ...
  const sortData = (key) => {
    if (key !== 4) {
      // Only sort the Overall Rating Score column (index 4)
      return;
    }
  ...
  const downloadExcel = () => {
    const headers = ['First Name', 'Last Name', 'Roll No', 'Reg. ID', 'Overall Rating Score', 'Action'];
    const worksheet = XLSX.utils.aoa_to_sheet([headers, ...sortedData]);
        ...
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
            <tr>
              ...
              >
                Overall Rating Score {sortConfig.key === 4 ? (sortConfig.direction === 'asc' ? '🔽' : '🔼') : ''}
              </th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((rowData, rowIndex) => (
              <tr key={rowIndex} style={{ border: '1px solid black' }}>
                ...
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[4]}</td> {/* Overall Rating Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>
