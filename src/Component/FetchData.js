...
//                Sorting Academics Score {sortOrder === 'asc' ? '🔽' : '🔼'}
//               </th>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cleanedData = rawData.map(row => row.slice(0, 6)); // Remove unwanted columns
        ...
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
            <tr>
              ...
              >
                Academics Score {sortConfig.key === 4 ? (sortConfig.direction === 'asc' ? '🔽' : '🔼') : ''}
              </th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((rowData, rowIndex) => (
              <tr key={rowIndex} style={{ border: '1px solid black' }}>
                ...
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[4]}</td> {/* Academics Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>
