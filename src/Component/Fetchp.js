...
//       try {
//         const response = await fetch('http://localhost:3001/api/data12');
//         if (!response.ok) {
...
//                Sorting Amcat Score {sortOrder === 'asc' ? '🔽' : '🔼'}
//               </th>
...
const Fetchp = () => {
  ...
  const sortData = (key) => {
    if (key !== 4) {
      // Only sort the Amcat Exam Score column (index 4)
      return;
    }
  ...
  const downloadExcel = () => {
    const headers = ['First Name', 'Last Name', 'Roll No', 'Reg. ID', 'Amcat Exam Score', 'Action'];
        ...
        <table style={{ borderCollapse: 'collapse', marginTop: '50px', width: '100%' }}>
          <thead>
            <tr>
              ...
              >
                Amcat Exam Score {sortConfig.key === 4 ? (sortConfig.direction === 'asc' ? '🔽' : '🔼') : ''}
              </th>
              <th style={{ border: '1px solid black', padding: '8px', textAlign: 'left' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {sortedData.map((rowData, rowIndex) => (
              <tr key={rowIndex} style={{ border: '1px solid black' }}>
                ...
                <td style={{ border: '1px solid black', padding: '8px' }}>{rowData[4]}</td> {/* Amcat Exam Score */}
                <td style={{ border: '1px solid black', padding: '8px' }}>
